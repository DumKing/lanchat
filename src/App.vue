...
onUnmounted(() => {
  ...
  Object.values(cameraFacePreviewUrls.value).forEach((url) => URL.revokeObjectURL(url));
  cameraFacePreviewUrls.value = {};
  clearCallSession();
  unlistenFaceMonitorMediaStatus?.();
  unlistenFaceMonitorMediaStatus = null;
  unlistenFaceMonitorFrames?.();
  unlistenFaceMonitorFrames = null;
...
function releaseCallMedia() {
  ...
  if (callSession.value) cameraMediaCoordinator.releaseCall(callSession.value.media);
...
async function prepareLocalCallMedia(media: CallMedia) {
  try {
    callLocalStream = await cameraMediaCoordinator.acquireForCall(media);
  } catch (error) {
...
const FACE_MONITOR_SETTINGS_STORAGE_KEY = "lanchat.face-monitor-settings.v1";

function readSavedFaceMonitorSettings(): CameraMonitorSettings {
...
function saveFaceMonitorSettingsLocally(settings: CameraMonitorSettings) {
  if (typeof window !== "undefined") {
    window.localStorage.setItem(FACE_MONITOR_SETTINGS_STORAGE_KEY, JSON.stringify(settings));
  }
}

async function updateCameraLivePreview(enabled: boolean) {
  cameraLivePreviewEnabled.value = enabled;
  if (!enabled) {
    if (cameraLivePreviewVideo.value) cameraLivePreviewVideo.value.srcObject = null;
    cameraMediaCoordinator.releasePreview();
    return;
  ...
  try {
    const stream = await cameraMediaCoordinator.acquirePreview();
    await nextTick();
    if (cameraLivePreviewVideo.value) {
      cameraLivePreviewVideo.value.srcObject = stream;
      await cameraLivePreviewVideo.value.play().catch(() => undefined);
    }
  } catch (error) {
    cameraLivePreviewEnabled.value = false;
    faceMonitorMediaStatus.value = { ...faceMonitorMediaStatus.value, lastError: formatCallMediaPermissionError(error, "video") };
  }
}
...
async function refreshCameraDeviceOptions() {
  ...
  try {
    ...
    cameraDeviceOptions.value = devices
      .filter((device) => device.kind === "videoinput")
      .map((device, index) => ({ label: device.label || `摄像头 ${index + 1}`, value: device.deviceId }));
  } catch (error) {
    faceMonitorMediaStatus.value = { ...faceMonitorMediaStatus.value, lastError: stringifyError(error) };
  }
}
...
async function attachLocalCameraFacePreview(record: CameraFaceAlert, sample: VisionFrameSample) {
  ...
  while (entries.length > 8) {
    const [expiredId, expiredUrl] = entries.shift() ?? [];
    if (expiredId && expiredUrl) {
...
const activeCameraFaceAlert = computed(() => cameraFaceAlerts.value.find((item) => item.alertId === activeCameraFaceAlertId.value) ?? null);
const activeCameraFacePreviewUrl = computed(() => activeCameraFaceAlert.value ? cameraFacePreviewUrls.value[activeCameraFaceAlert.value.alertId] ?? "" : "");

function closeCameraFaceAlertPreview() {
  cameraFaceAlertPreviewOpen.value = false;
}
...
function scheduleFaceMonitorRuntimeStatusRefresh() {
  if (faceMonitorStatusTimer !== null || typeof window === "undefined") return;
  faceMonitorStatusTimer = window.setTimeout(() => {
    faceMonitorStatusTimer = null;
    void refreshFaceMonitorRuntimeStatus();
  }, 2000);
}

async function updateFaceMonitorSettings(next: Partial<CameraMonitorSettings>) {
  const settings: CameraMonitorSettings = {
  ...
  try {
    ...
    faceMonitorSaving.value = false;
  }
}

async function initializeFaceMonitor() {
  const saved = readSavedFaceMonitorSettings();
  faceMonitorSettings.value = saved;
...
const facePolicyTargetOptions = computed(() => [
  { label: "所有在线设备", value: "*" },
  ...peers.value.filter((peer) => peer.online).map((peer) => ({ label: `${peerDisplayName(peer)} · ${peer.address}`, value: peer.device_id })),
]);
...
async function openLocalFaceCapture() {
  if (!navigator.mediaDevices?.getUserMedia) {
    store.error = "当前环境不支持摄像头拍照";
    return;
  }
  try {
    localFaceCaptureStream = await cameraMediaCoordinator.acquirePreview();
    localFaceCaptureOpen.value = true;
    await nextTick();
    if (localFaceCaptureVideo.value) {
      localFaceCaptureVideo.value.srcObject = localFaceCaptureStream;
      await localFaceCaptureVideo.value.play();
    }
  } catch (error) {
    store.error = formatCallMediaPermissionError(error, "video");
  }
}

function closeLocalFaceCapture() {
  localFaceCaptureStream = null;
  cameraMediaCoordinator.releasePreview();
  if (localFaceCaptureVideo.value) localFaceCaptureVideo.value.srcObject = null;
  localFaceCaptureOpen.value = false;
}

async function captureLocalFacePhoto() {
  const video = localFaceCaptureVideo.value;
...
<template>
  <NConfigProvider :theme-overrides="themeOverrides" :locale="naiveLocale" :date-locale="dateLocale" :class="['provider-root', selectedTheme]">
    <NMessageProvider>
      ...
      <NModal
        :show="cameraFaceAlertPreviewOpen"
        preset="card"
        class="camera-face-alert-modal"
        :mask-closable="true"
      ...
      >
        <div v-if="activeCameraFaceAlert" class="camera-face-alert-detail">
          ...
          <NSpace justify="end"><NButton type="primary" @click="closeCameraFaceAlertPreview">我知道了</NButton></NSpace>
        </div>
      </NModal>
      <Teleport to="body">
        <aside
                ...
                <NCard class="camera-face-settings-card" title="摄像头与识别策略" size="small">
                  <NSpace vertical>
                    ...
                    <NFormItem label="摄像头" :show-feedback="false">
                      <NSelect
                        :value="faceMonitorSettings.deviceId ?? null"
                        :options="cameraDeviceOptions"
                        placeholder="使用系统默认摄像头"
                        clearable
                        :disabled="faceMonitorSaving"
                        @update:value="(value) => updateFaceMonitorSettings({ deviceId: value || null })"
                      />
                    </NFormItem>
                    <div class="setting-switch-row compact-setting-row">
                      <div>
                        <strong>视频通话期间暂停识别</strong>
                        <p>关闭时，视频通话期间自动降到每秒 1 帧。</p>
                      </div>
                      <NSwitch :value="faceMonitorSettings.pauseDuringCall" :disabled="!faceMonitorSettings.enabled || faceMonitorSaving" @update:value="(value) => updateFaceMonitorSettings({ pauseDuringCall: value })" />
                    </div>
                    <div class="setting-switch-row compact-setting-row">
                      <div>
                        <strong>显示实时检测画面</strong>
                        <p>仅在此页面显示本机摄像头预览，不会录制、保存或发送。</p>
                      </div>
                      <NSwitch :value="cameraLivePreviewEnabled" :disabled="faceMonitorSaving" @update:value="updateCameraLivePreview" />
                    </div>
                    ...
                    <div class="face-monitor-people">
                      ...
                      <div v-for="alert in cameraFaceAlerts.slice(0, 5)" :key="alert.alertId" class="face-monitor-alert-row">
                        ...
                        <NSpace :size="6">
                          ...
                          <NButton size="tiny" secondary type="error" @click="feedbackCameraFaceAlert(alert, 'false')">虚假 {{ alert.feedbackFalse }}</NButton>
                        </NSpace>
                      </div>
                    </div>
                    <NAlert v-if="faceMonitorMediaStatus.lastError" type="warning" :show-icon="false">{{ faceMonitorMediaStatus.lastError }}</NAlert>
                    <NAlert v-if="faceMonitorRuntimeStatus?.lastError" type="info" :show-icon="false">{{ faceMonitorRuntimeStatus.lastError }}</NAlert>
      ...
      <NModal v-model:show="localFaceCaptureOpen" preset="card" title="摄像头采集参考照片" class="face-capture-modal" @after-leave="closeLocalFaceCapture">
        <NSpace vertical>
          <video ref="localFaceCaptureVideo" class="face-capture-video" autoplay muted playsinline></video>
          <NText depth="3">请让人脸位于画面中央、光线充足。拍摄结果仅保存到本机人员库。</NText>
...
.monopoly-layout { --monopoly-player-1: #e45a5a; --monopoly-player-2: #377fe8; --monopoly-player-3: #3ca56a; --monopoly-player-4: #9a62d6; position: relative; display: grid; grid-template-columns: minmax(0, 1fr); grid-template-rows: minmax(0, 1fr); min-height: 0; overflow: hidden; }
...
.monopoly-property-building i { position: absolute; display: block; box-sizing: border-box; }.monopoly-building-roof { z-index: 3; top: 0; left: 3px; width: 21px; height: 11px; border: 1px solid color-mix(in srgb, var(--monopoly-building-color) 78%, #3d4654); border-radius: 4px 4px 1px 1px; background: linear-gradient(135deg, color-mix(in srgb, var(--monopoly-building-color) 64%, #fff), var(--monopoly-building-color) 56%, color-mix(in srgb, var(--monopoly-building-color) 62%, #19212c)); clip-path: polygon(50% 0, 100% 85%, 0 85%); }.monopoly-building-body { z-index: 2; right: 5px; bottom: 1px; left: 5px; height: 12px; border: 1px solid color-mix(in srgb, var(--monopoly-building-color) 70%, #465362); border-radius: 2px 2px 4px 4px; background: linear-gradient(90deg, color-mix(in srgb, var(--monopoly-building-color) 46%, #fff), var(--monopoly-building-color) 65%); }.monopoly-building-door { z-index: 4; bottom: 1px; left: 11px; width: 5px; height: 7px; border-radius: 2px 2px 1px 1px; background: #fff5d7; box-shadow: inset -2px 0 color-mix(in srgb, var(--monopoly-building-color) 58%, #495563); }.monopoly-building-window { z-index: 4; right: 7px; bottom: 7px; width: 4px; height: 4px; border-radius: 1px; background: #e9f8ff; box-shadow: -8px 0 #e9f8ff; }
.monopoly-building-empty { height: 14px; margin-top: 6px; filter: none; transform: none; }.monopoly-building-empty .monopoly-building-roof, .monopoly-building-empty .monopoly-building-door, .monopoly-building-empty .monopoly-building-window { display: none; }.monopoly-building-empty .monopoly-building-body { right: 1px; bottom: 1px; left: 1px; height: 10px; border-color: #9aa2ac; border-radius: 50% 50% 5px 5px; background: repeating-linear-gradient(135deg, #d8dde2 0 4px, #bcc4cd 4px 8px); box-shadow: inset 0 -3px #a7afb9; }
.monopoly-building-house { width: 29px; height: 24px; }.monopoly-building-level2 { width: 34px; height: 29px; }.monopoly-building-level2 .monopoly-building-roof { left: 4px; width: 26px; height: 12px; }.monopoly-building-level2 .monopoly-building-body { right: 4px; left: 4px; height: 17px; }.monopoly-building-level2 .monopoly-building-door { left: 14px; height: 9px; }.monopoly-building-level2 .monopoly-building-window { right: 7px; bottom: 10px; box-shadow: -10px 0 #e9f8ff, 0 6px #e9f8ff, -10px 6px #e9f8ff; }
.monopoly-building-level3 { width: 37px; height: 34px; }.monopoly-building-level3 .monopoly-building-roof { top: 0; left: 12px; width: 13px; height: 11px; border-radius: 7px 7px 1px 1px; clip-path: none; }.monopoly-building-level3 .monopoly-building-body { right: 7px; left: 7px; height: 27px; border-radius: 4px 4px 5px 5px; background: linear-gradient(90deg, color-mix(in srgb, var(--monopoly-building-color) 35%, #fff), var(--monopoly-building-color) 58%, color-mix(in srgb, var(--monopoly-building-color) 72%, #27313c)); }.monopoly-building-level3 .monopoly-building-door { left: 16px; height: 10px; }.monopoly-building-level3 .monopoly-building-window { right: 10px; bottom: 14px; box-shadow: -9px 0 #e9f8ff, 0 7px #e9f8ff, -9px 7px #e9f8ff; }
.monopoly-building-player-1 { --monopoly-building-color: var(--monopoly-player-1); }.monopoly-building-player-2 { --monopoly-building-color: var(--monopoly-player-2); }.monopoly-building-player-3 { --monopoly-building-color: var(--monopoly-player-3); }.monopoly-building-player-4 { --monopoly-building-color: var(--monopoly-player-4); }.monopoly-building-neutral { --monopoly-building-color: #8c96a4; }
.monopoly-tile .monopoly-property-building { width: calc(100% - 2px); height: calc(100% - 2px); max-width: none; max-height: none; right: 1px; bottom: 1px; left: 1px; background-image: var(--monopoly-building-sheet); background-position: var(--monopoly-building-column) var(--monopoly-building-row); background-repeat: no-repeat; background-size: 500% 300%; filter: drop-shadow(0 2px 2px #2632444d); transform: none; }
.monopoly-tile .monopoly-property-building.monopoly-building-level2, .monopoly-tile .monopoly-property-building.monopoly-building-level3 { transform: none; }
.monopoly-tile .monopoly-property-building.monopoly-building-empty { width: calc(100% - 2px); height: calc(100% - 2px); right: 1px; bottom: 1px; left: 1px; border: 0; border-radius: 0; background: transparent; box-shadow: none; filter: none; transform: none; }
.monopoly-empty-lot { position: absolute; inset: 2px 1px 0; display: block; clip-path: polygon(50% 0, 100% 50%, 50% 100%, 0 50%); background: linear-gradient(135deg, #e9edf1 0 48%, #bac4ce 49% 100%); box-shadow: inset 0 0 0 2px #7d8995, inset 0 -5px 0 #98a4af, 0 3px 4px #26324455; }
.monopoly-empty-lot::before, .monopoly-empty-lot::after { position: absolute; content: ""; width: 5px; height: 5px; border-radius: 50%; background: #6a9c62; box-shadow: 0 1px 1px #1e553633; }
.monopoly-empty-lot::before { top: 22%; left: 24%; }.monopoly-empty-lot::after { right: 24%; bottom: 22%; }
.monopoly-event-slot-machine { position: absolute; z-index: 1; right: 1px; bottom: 1px; left: 1px; display: block; width: calc(100% - 2px); height: calc(100% - 2px); object-fit: cover; object-position: center; pointer-events: none; filter: drop-shadow(0 2px 2px #6b1f224d); }.monopoly-tile.event .monopoly-event-slot-machine { animation: monopoly-slot-machine-idle 1.25s ease-in-out infinite alternate; }
.monopoly-corner-landmark { position: absolute; z-index: 1; inset: 1px; display: block; width: auto; height: auto; background-image: url('/games/monopoly/corners/corner-landmarks.png'); background-repeat: no-repeat; background-size: 200% 200%; filter: drop-shadow(0 2px 2px #26324455); pointer-events: none; }
...
.monopoly-rule-god-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; margin-top: 12px; }.monopoly-rule-god { display: grid; grid-template-columns: 64px minmax(0, 1fr); align-items: center; min-width: 0; gap: 10px; padding: 10px; border: 1px solid var(--panel-border); border-radius: 8px; background: var(--input-bg); }.monopoly-rule-god-preview { display: block; width: 58px; height: 58px; filter: drop-shadow(0 3px 4px #26324455); }.monopoly-rule-god strong { color: var(--text-primary); font-size: 13px; }.monopoly-rule-god p { margin: 4px 0 0; color: var(--text-secondary); font-size: 11px; line-height: 1.45; }
...
@keyframes monopoly-target-glow { from { box-shadow: inset 0 0 0 1px var(--accent); } to { box-shadow: inset 0 0 0 4px color-mix(in srgb, var(--accent) 48%, transparent); } }
@keyframes monopoly-event-reel { from { transform: translateY(-1px) rotate(-7deg); } to { transform: translateY(1px) rotate(7deg); } }
...
/* Shared board item styling for the flat and pseudo-3D boards. */
.monopoly-empty-lot { inset: 5px 4px 3px; clip-path: polygon(50% 0, 100% 50%, 50% 100%, 0 50%); background: linear-gradient(135deg, color-mix(in srgb, var(--accent) 15%, var(--panel-bg)) 0 50%, color-mix(in srgb, var(--accent) 25%, var(--line)) 50% 100%); box-shadow: inset 0 -4px 0 color-mix(in srgb, var(--accent) 30%, var(--line)), 0 3px 5px #26324438; }
.monopoly-empty-lot::before { content: ""; position: absolute; inset: 18%; width: auto; height: auto; border: 1px dashed color-mix(in srgb, var(--accent) 52%, var(--panel-bg)); border-radius: 0; clip-path: polygon(50% 0, 100% 50%, 50% 100%, 0 50%); background: transparent; box-shadow: none; }
.monopoly-empty-lot::after { content: "⌂"; position: absolute; left: 50%; top: 47%; right: auto; bottom: auto; display: grid; place-items: center; width: 16px; height: 12px; transform: translate(-50%, -50%); border: 1px solid color-mix(in srgb, var(--accent) 48%, var(--line)); border-radius: 3px; background: var(--panel-bg); color: var(--accent); font-size: 10px; font-weight: 900; line-height: 1; box-shadow: 0 1px 3px #26324433; }
.monopoly-player-seat.jailed { border-color: #758394; box-shadow: inset 0 0 0 2px #8593a326, 0 8px 20px #28384b2b; }
.monopoly-player-jail-chains { position: absolute; inset: 0; z-index: 8; overflow: hidden; border-radius: inherit; pointer-events: none; }
.monopoly-player-jail-chains svg { display: block; width: 100%; height: 100%; overflow: visible; filter: drop-shadow(0 2px 2px #18243370); }
.monopoly-player-jail-chain-rail { fill: none; stroke: #536170; stroke-width: 6; vector-effect: non-scaling-stroke; }
.monopoly-player-jail-chain-links { fill: none; stroke: #dce4eb; stroke-width: 3.5; stroke-linecap: round; stroke-dasharray: 2 5; vector-effect: non-scaling-stroke; animation: monopoly-jail-chain-glint 2.4s linear infinite; }
.monopoly-player-jail-lock { position: absolute; top: 50%; left: 50%; display: grid; width: calc(24 * var(--monopoly-u, 1px)); height: calc(24 * var(--monopoly-u, 1px)); place-items: center; border: 2px solid #566474; border-radius: calc(6 * var(--monopoly-u, 1px)); background: linear-gradient(135deg, #f4f7fa, #aeb9c6); box-shadow: 0 3px 7px #1b27366b, inset 0 1px #fff; color: #465362; font-family: KaiTi, serif; font-size: calc(12 * var(--monopoly-u, 1px)); font-weight: 900; transform: translate(-50%, -50%); }
@keyframes monopoly-jail-chain-glint { to { stroke-dashoffset: -14; } }
.monopoly-god-avatar { display: inline-block; box-sizing: border-box; background-image: url('/games/monopoly/items/board-items.png'); background-repeat: no-repeat; background-size: 300% 200%; color: transparent; font-size: 0; text-shadow: none; filter: drop-shadow(0 1px 2px #26324455); }
.monopoly-god-avatar-wealth { background-position: 0 100%; }.monopoly-god-avatar-poverty { background-position: 50% 100%; }.monopoly-god-avatar-angel { background-position: 50% 0; }.monopoly-god-avatar-devil { background-position: 100% 0; }
</style>
