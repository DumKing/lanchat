...
const basicSettings = await readFile(new URL("../src/pages/settings/BasicSystemSettings.vue", import.meta.url), "utf8");
...
assert.match(settingsView, /title="图片缓存"[^>]*class="basic-image-cache-card"/, "图片缓存必须标记为末尾工具卡片");
assert.match(settingsView, /title="网络修复"[^>]*class="basic-network-repair-card"/, "网络修复必须标记为末尾工具卡片");
...
assert.match(settingsView, /title="版本更新"[\s\S]*GitHub API Token/, "版本更新和 GitHub Token 必须合并到同一卡片");
assert.doesNotMatch(settingsView, /title="GitHub API Token"/, "GitHub Token 不应继续占用独立卡片");
