...
- 已完成：V5 追加式迁移、Manifest V3 校验、签名与缓存边界、Latest Frame Worker、短时追踪/多样本匹配、策略去重协议、Raw RGBA IPC、视觉工作区和中英文文案。
- 已完成：人员库的新录入约束（3 至 30 张、多人图拦截）、本地私有参考图清理、运行时暂停/活动 Profile 持久化、P50/P95 与队列诊断。
- 兼容策略：历史 `face_*` 表、旧人员与旧告警保持可读；旧 JPEG 命令保留为 Rust 侧兼容入口，前端不再主动编码或发送 JPEG。
- 发布前仍需人工验收：普通笔记本上的实际 P95/内存、摄像头切换、通话共享、模型安装/回退、局域网重复策略投递。
