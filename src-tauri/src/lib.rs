...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // 现有 ONNX Profile，但保留原因以便从 Debug 日志定位资源布局问题。
            if let Err(error) =
                vision::openvino_runtime::configure_packaged_runtime(resource_dir.as_deref())
            {
                eprintln!("OpenVINO Runtime 预加载失败: {error}");
            }
